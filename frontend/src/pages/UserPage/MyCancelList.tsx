...
import { getMyCancelReservation } from "@api/mypage";
import { PopupProps } from "@api/home";

const MyCancelList = () => {
  const [isModal, setIsModal] = useState(false);
  const [modalPopup, setModalPopup] = useState<PopupProps | null>(null);
  const [cancelledPopups, setCancelledPopups] = useState<PopupProps[]>([]);

  // API 호출하여 취소된 팝업 데이터 가져오기
  useEffect(() => {
    getMyCancelReservation()
      .then((data) => {
        setCancelledPopups(data);
      })
      .catch((error) => {
        console.error("Failed to fetch cancelled reservations:", error);
      });
  }, []);

  const openModal = (popup: PopupProps) => {
    setIsModal(true);
    setModalPopup(popup);
