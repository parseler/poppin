...
import { getMyCancelReservation, ReservationProps, getMyPreReservationDetail } from "@api/mypage";
import { getUserData } from "@api/users";

interface MyCancelListProps {
    reservations?: ReservationProps[];
}

const baseUrl = "http://localhost";

const MyCancelList: React.FC<MyCancelListProps> = ({ reservations: initialReservations = [] }) => {
    const [isModal, setIsModal] = useState(false);
    const [modalReservation, setModalReservation] = useState<ReservationProps | null>(null);
    const [cancelledReservations, setCancelledReservations] = useState<ReservationProps[]>(initialReservations);
    const [userPhoneNumber, setUserPhoneNumber] = useState<string>("");

    useEffect(() => {
        getMyCancelReservation()
            .then((data) => {
                setCancelledReservations(data);
            })
            .catch((error) => {
                console.error("취소된 예약을 가져오는데 실패했습니다:", error);
                setCancelledReservations([]);  // 에러 발생 시 빈 배열로 설정
            });
    }, []);
    ...
    const openModal = async (reservationId: number) => {
        try {
            ...
            console.error("예약 상세 정보를 가져오는데 실패했습니다:", error);
        }
    };
                        ...
                        <Box id="reservation-modal-box" onClick={(e) => e.stopPropagation()}>
                            <div className="modal-overlay">
                                ...
                                <div className="reservation-info">
                                    ...
                                    <p className="date">
                                        취소 확정일: {modalReservation.reservationDate}
                                    </p>
                                    <p className="time">
