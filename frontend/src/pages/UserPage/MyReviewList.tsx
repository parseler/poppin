...
const MyReviewList = () => {
    ...
    }, []); // Remove thumbnails from dependency array to prevent infinite loop
            ...
            <div className="my-review-content">
                {writeReviews.length > 0 ? (
                    writeReviews.map((review) => {
                        console.log("Review ID:", review.reviewId, "Thumbnail URL:", review.thumbnail);
                        return (
                            <Link key={review.reviewId} to={`/review/${review.reviewId}`}>
                                <ReviewMedium
                                    reviewId={review.reviewId}
                                    nickname={review.nickname}
                                    img={getImageUrl(review.img)}
                                    thumbnail={getImageUrl(review.thumbnail)}
                                    title={review.title}
